...
module.exports = async (req, res) => {
  ...
  const data = {
    ...
    absen_masuk: waktuIndonesia,
    latitude: req.body.latitude,
    longitude: req.body.longitude,
    lokasi_link: req.body.lokasi_link,
  };
  ...
  if (result > 100) {
    ...
    const createdAbsen = await Presensi.create(data);
    data.lokasi_link =
      lokasiKerja.lokasi + "/" + data.latitude + "," + data.longitude;

    return res.json({
      status: "success",
      message: "anda berhasil absen, jangan lupa untuk absen pulang ya!",
      data: {
        id: createdAbsen.id,
        user_id: data.user_id,
        absen_masuk: data.absen_masuk,
        lokasi_link: data.lokasi_link,
